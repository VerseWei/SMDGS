...
def processing_single_scene(args):
    ...
    model_dir = os.path.join(args.data_path, 'sparse/0')
    ...
    for i in range(num_images):
        ...
        with open(os.path.join(cam_dir, '%s_cam.txt' % id), 'w') as f:
            f.write('extrinsic\n')
            for j in range(4):
                for k in range(4):
    ...
    with open(os.path.join(args.save_folder, 'pair.txt'), 'w') as f:
        ...
        for i, sorted_score in enumerate(view_sel):
            id = images[i+1].name.split('.')[0]
            f.write('%d %s\n%d ' % (i, id, len(sorted_score)))
            for image_id, s in sorted_score:
                id = images[image_id+1].name.split('.')[0]
                f.write('%d %s %d ' % (image_id, id, s))
            f.write('\n')
    ...
    for i in range(num_images):
        ...
        if not img_path.endswith(".jpg"):
            ...
            cv2.imwrite(os.path.join(image_converted_dir, '%s.jpg' % id), img)
        else:
            shutil.copyfile(os.path.join(image_dir, images[i+1].name), os.path.join(image_converted_dir, '%s.jpg' % id))
    # shutil.copytree(image_dir, os.path.join(args.save_folder, 'images'))
...
if __name__ == '__main__':
    ...
    args.save_folder = args.save_folder if args.save_folder is not None else  '{}/hpm_dense'.format(args.data_path)
