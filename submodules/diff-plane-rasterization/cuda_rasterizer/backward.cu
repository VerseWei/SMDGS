...
renderCUDA(
	...
	const float* __restrict__ final_Ts_s,
	const float* __restrict__ final_Ns,
	...
	const float* __restrict__ dL_dpixopac_s,
	...
	float* config)
{
	...
	float T_final_s = inside ? final_Ts_s[pix_id] : 0;
	float T = T_final;          // 最末端的透光度T

	float N_final[3];
	for (int ch = 0; ch < 3; ch++) {
		N_final[ch] = inside ? final_Ns[pix_id * 3 + ch] : 0;
	}
	...
	if (inside) {
		...
		dL_dpixO_s = dL_dpixopac_s[pix_id];
		if(render_geo) {
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float dchannel_dallmap = alpha * T;
			...
			if (render_geo && scene_point) {
				for (int ch = 0; ch < MAP_N; ch++)
				{
					...
					float dL_dchannel;
					dL_dchannel = dL_dout_all_map[ch];
					
					dL_dalpha += (c - accum_all_map[ch]) * dL_dchannel;
					// Update the gradients w.r.t. color of the Gaussian. 
					// Atomic, since this pixel is just one of potentially
					// many that were affected by this Gaussian.
					atomicAdd(&(dL_dall_map[global_id * MAP_N + ch]), dchannel_dallmap * dL_dchannel);
				}
			}
			...
			{	
				...
				if (scene_point) {
					dL_dalpha += dL_dpixO_s * T_final_s / (1 - alpha);
				}
			}
...
void BACKWARD::render(
	...
	const float* final_Ts_s,
	const float* final_Ns,
