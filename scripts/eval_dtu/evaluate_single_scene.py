...
def cull_scan(scan, mesh_path, result_mesh_file, instance_dir, mask_cull=False):
    ...
    # load poses
    print(instance_dir)
    image_dir = '{0}/colmap/images'.format(instance_dir)
    ...
    try:
        cameras_extrinsic_file = os.path.join(instance_dir, "colmap/sparse/0", "images.bin")
        cameras_intrinsic_file = os.path.join(instance_dir, "colmap/sparse/0", "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
        cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
    except:
        cameras_extrinsic_file = os.path.join(instance_dir, "colmap/sparse/0", "images.txt")
        cameras_intrinsic_file = os.path.join(instance_dir, "colmap/sparse/0", "cameras.txt")
        cam_extrinsics = read_extrinsics_text(cameras_extrinsic_file)
        cam_intrinsics = read_intrinsics_text(cameras_intrinsic_file)
    H, W, fx, fy, cx, cy = cam_intrinsics[1].height, cam_intrinsics[1].width, cam_intrinsics[1].params[0], \
            cam_intrinsics[1].params[1], cam_intrinsics[1].params[2], cam_intrinsics[1].params[3]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root_dir', type=str, help='the root path of data to be evaluated')
    ...
    cull_scan(scan, ply_file, result_mesh_file, instance_dir=os.path.join(args.root_dir, f'scan{args.scan_id}'))
